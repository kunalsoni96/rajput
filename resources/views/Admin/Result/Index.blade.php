...
<div class="col-sm-6">
<a href="{{route('Admin/Result/Add')}}" style="float: right; margin-top: -10px;" class="btn btn-danger">Add New</a>
</div>
</div>
</div>
...
<tr>
    ...
    <td>{{$row->Rank}}</td>
    <td>
    	<img src="{{url('/')}}/public/result/{{$row->Image}}" style="width: 50px; height: 50px">
    </td>
    <td>
    	@if($row->Status=='1')
    ...
    <form action="{{route('Admin/Result/EditSession')}}" method="post">
    <td>
    	{{csrf_field()}}
    	...
    	<form action="{{route('Admin/Result/Delete')}}" method="post">
    		{{csrf_field()}}
