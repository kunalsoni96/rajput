...
<div class="col-sm-6">
<a href="{{route('Admin/Images/Add')}}" style="float: right; margin-top: -10px;" class="btn btn-danger">Add New</a>
</div>
</div>
</div>
...
@foreach($data as $row)
    <tr>
        ...
        <td>{{$row->Category}}</td>
        <td>
        	<img src="{{url('/')}}/public/gallery/{{$row->Image}}" style="width: 50px; height: 50px">
        </td>
        <td>
        	@if($row->Status=='1')
        ...
        <form action="{{route('Admin/Images/EditSession')}}" method="post">
        <td>
        	{{csrf_field()}}
